...
def http_test(test, host, force):
  ...
  if 'username' in test and 'password' in test:
    ...
    curlurl = '{0}://{1}:{2}@{3}:{4}{5}'.format(test['protocol'], username, password, host, test['port'], test['url'])
  ...
  if 'expect_status_code' in test:
    if 'response_redirect' in test and (req.status_code == 301 or req.status_code == 302 or req.status_code == 307 or req.status_code == 308):
      ...
      if int(test['expect_status_code']) == req.status_code:
        ...
        fail_test("Expecting status code {0} but got {1} instead".format(test['expect_status_code'], str(req.status_code)))
  else:
  ...
  if 'response_max_time_ms' in test:
    if duration_ms > int(test['response_max_time_ms']):
      ...
      fail_test('Response time was {0}ms longer than {1}ms max ({2}ms)'.format(str(longer_by_ms), test['response_max_time_ms'], str(duration_ms)))
    else:
  ...
  if 'response_json_contains' in test:
    for path in test['response_json_contains'].keys():
      ...
      if expected_value == actual_value:
        ...
        fail_test("Invalid json value {0} at path \"{1}\", expecting {2}".format(actual_value, path, expected_value))
